...
import (
	...
	"github.com/cocoyes/go-captcha/base/imagedata"
	"github.com/cocoyes/go-captcha/base/logger"
	"github.com/cocoyes/go-captcha/base/option"
	"github.com/cocoyes/go-captcha/base/randgen"
	"github.com/cocoyes/go-captcha/base/random"
)
...
func (c *captcha) Generate(group string) (CaptchaData, error) {
	if c.mode == ModeShape {
		return c.generateWithShape()
	}

	return c.generateWithText(group)
}
...
//   - CaptchaData: Generated captcha data
//   - error: Error information
func (c *captcha) generateWithText(group string) (CaptchaData, error) {
	if err := c.check(); err != nil {
		return nil, err
	}

	chars, err := c.genChars(group)
	if err != nil {
		return nil, err
	}
...
//   - error: Error information
func (c *captcha) genChars(group string) ([]string, error) {
	length := random.RandInt(c.opts.rangeLen.Min, c.opts.rangeLen.Max)
	chars := c.genRandChar(group, length)
	if len(chars) == 0 {
...
func (c *captcha) genRandChar(group string, length int) []string {
	var strA []string
	for len(strA) < length {
		char := randgen.RandString(c.resources.chars[group])
